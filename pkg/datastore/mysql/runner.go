...
func (m *MySQL) CreateRunner(ctx context.Context, runner datastore.Runner) error {
	...
	queryDetail := `INSERT INTO runner_detail(runner_id, shoes_type, ip_address, target_id, cloud_id, repository_url, request_webhook) VALUES (?, ?, ?, ?, ?, ?, ?)`
	if _, err := tx.ExecContext(ctx, queryDetail, runner.UUID.String(), runner.ShoesType, runner.IPAddress, runner.TargetID.String(), runner.CloudID, runner.RepositoryURL, runner.RequestWebhook); err != nil {
		tx.Rollback()
...
func (m *MySQL) ListRunners(ctx context.Context) ([]datastore.Runner, error) {
	...
	query := `SELECT runner.runner_id, detail.shoes_type, detail.ip_address, detail.target_id, detail.cloud_id, detail.created_at, detail.updated_at, detail.repository_url, detail.request_webhook
